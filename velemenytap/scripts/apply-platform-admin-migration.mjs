...
const migrationName = '20260920160000_platform_card_admin.sql';
...
try {
  ...
  await db.query("select pg_advisory_xact_lock(hashtext('velemenytap-platform-admin-migration'))");
  ...
  const alreadyApplied = history.includes('20260920160000');
  if (JSON.stringify(history.filter(v => v !== '20260920160000')) !== JSON.stringify(expected)) throw Error('Migration history differs from expected baseline; refusing to apply');
  ...
  if (!alreadyApplied && applyFlag === '--apply') {
    ...
    await db.query(sql);
    await db.query('insert into supabase_migrations.schema_migrations(version,name,statements) values ($1,$2,$3)', ['20260920160000','platform_card_admin',[sql]]);
  }
