...
const migrationName = '20260921150000_billing_card_monitor.sql';
...
try {
  ...
  await db.query("select pg_advisory_xact_lock(hashtext('velemenytap-billing-monitor-migration'))");
  ...
  const alreadyApplied = history.includes('20260921150000');
  if (JSON.stringify(history.filter(v => v !== '20260921150000')) !== JSON.stringify(expected)) throw Error('Migration history differs from expected baseline; refusing to apply');
  ...
  if (!alreadyApplied && applyFlag === '--apply') {
    ...
    await db.query(sql);
    await db.query('insert into supabase_migrations.schema_migrations(version,name,statements) values ($1,$2,$3)', ['20260921150000','billing_card_monitor',[sql]]);
  }
