...
The git repository root (one level above this file's directory) also contains unrelated files (design assets, `.claude/` skill config) — the actual Next.js application lives entirely inside `velemenytap/`. `.github/workflows/` lives at the true repo root (GitHub requires this), one level above `velemenytap/`; every workflow job sets `working-directory: velemenytap` accordingly.
...
| `STRIPE_SECRET_KEY` | **live** key (`sk_live_…`) | **test** key (`sk_test_…`) | test key |
| `STRIPE_WEBHOOK_SECRET` | live endpoint's signing secret | test/preview endpoint's signing secret | `stripe listen`'s own secret |
| `STRIPE_PRICE_ID_MONTHLY` / `STRIPE_PRICE_ID_YEARLY` | **live**-mode Price ids | **test**-mode Price ids | test-mode Price ids |
| `RECONCILE_SWEEP_SECRET` | real, long random secret (owner action — not yet set) | unset unless deliberately testing the sweep against Preview | unset (the sweep isn't exercised locally) |

### Stripe test/live separation (second independent review, Finding 6)

**Production uses Stripe live mode; every other environment (Preview and local development) uses Stripe test mode — never mixed.** A live-mode key or Price id reaching a non-production environment could create a REAL subscription and charge a real card from a preview deployment or a developer's laptop; a test-mode key or Price id reaching production would silently accept "payments" that never actually charge anyone. Both directions are checked automatically and fail closed, at two different layers:

- **`features/billing/stripe-config.ts`'s `assertStripeConfigurationValid()`** — called before every Checkout attempt (`createCheckoutSessionAction`). Verifies `STRIPE_SECRET_KEY`'s own `sk_live_…`/`sk_test_…` prefix agrees with the runtime's expected livemode, and that both `STRIPE_PRICE_ID_MONTHLY`/`STRIPE_PRICE_ID_YEARLY` resolve (via a live `prices.retrieve` call) to Prices whose own `livemode` field also agrees, are `active`, charge exactly what the billing page promises (5,990 Ft / 59,900 Ft — compared in minor units, i.e. `599000`/`5990000` `unit_amount`, `huf`, `month`/`year` recurrence with `interval_count` exactly 1 — fourth independent review, Finding 13), have `tax_behavior: "inclusive"` (matching the page's "az árak az ÁFát tartalmazzák" copy — see "Stripe Tax / Hungarian VAT invoicing" below for what this check does and does not prove), and both resolve to the SAME Stripe Product (this app sells one product, two billing cadences — see `features/billing/plans.ts`). Memoized per server process; a failure is never cached, so a genuine misconfiguration keeps failing loudly on the next attempt rather than being silently retried into success once fixed.
- **`app/api/webhooks/stripe/route.ts`'s `checkEventEnvironment()`** — called on every webhook delivery. Verifies the event's own `livemode` field agrees with the runtime's expected livemode before processing anything; a mismatch is rejected with HTTP 400 and logged loudly, never silently ignored or silently applied.
- **What "the runtime's expected livemode" means, and when it's skipped (fourth independent review, Finding 6, HIGH):** both checks above go through the same `resolveExpectedLivemode()` (`features/billing/stripe-config.ts`), and it now **fails closed, not open**. The prior design enforced only when `APP_ENV` resolved to exactly `"production"` or `"preview"`, silently skipping enforcement for anything else — including `APP_ENV` genuinely unset in a deployed environment, which is precisely the misconfiguration this check exists to catch, not a case that should ever look the same as intentional local development. Fixed by inverting the condition: `NODE_ENV === "development"` (the one value `next dev` itself sets — the same signal `/api/health` already uses, see § 6) is now the *only* case that skips enforcement; every other runtime — a Vercel build, `next start`, a misconfigured environment with `APP_ENV` unset or malformed — must resolve `APP_ENV` to exactly `"production"` or `"preview"` or `resolveExpectedLivemode()` throws, and both call sites above refuse to proceed on that thrown error rather than crashing unhandled or silently continuing. **Operational consequence: an `APP_ENV` misconfiguration in a real deployment now blocks Checkout and every webhook delivery outright (loud, visible failure) instead of silently skipping the livemode check** — this is the intended, deliberate tradeoff (see `DECISIONS.md`), not a regression to investigate if a deployment with a missing `APP_ENV` starts failing Stripe operations after this change.

**Setting up a new environment's Stripe configuration:**
1. In the [Stripe dashboard](https://dashboard.stripe.com), toggle to the correct mode (Test/Live) in the top-left switcher before creating anything below — everything created while in the wrong mode is invisible to, and unusable from, the other.
2. Create one Product ("VéleményTap előfizetés" or similar) with two recurring Prices attached to it: monthly (5,990 Ft, `month`, tax-inclusive) and yearly (59,900 Ft, `year`, tax-inclusive). Both currency `HUF`.
3. Copy each Price's id (`price_…`) into `STRIPE_PRICE_ID_MONTHLY`/`STRIPE_PRICE_ID_YEARLY` for that environment's scope.
4. Copy the matching-mode secret key (Developers → API keys) into `STRIPE_SECRET_KEY`.
5. Create a webhook endpoint (Developers → Webhooks → Add endpoint) pointed at `https://<this-environment's-domain>/api/webhooks/stripe`, subscribed to exactly the events listed below, and copy its signing secret into `STRIPE_WEBHOOK_SECRET`. For local development, `stripe listen --forward-to localhost:3000/api/webhooks/stripe` prints its own temporary signing secret instead — use that, not a dashboard endpoint's.
6. Enable the [Billing Portal](https://dashboard.stripe.com/settings/billing/portal) (Settings → Billing → Customer portal) for that mode — `createPortalSessionAction` fails if no Portal configuration exists yet for the mode the active key resolves to. No specific feature toggles are required beyond the default (subscription cancellation and payment-method update are what this app's Portal button is for); leave "update subscription" plan-switching off unless a product decision explicitly adds a UI for it, since this app's own Checkout flow — not the Portal — is currently the only supported way to start or resubscribe.

**Webhook events this app subscribes to** (Developers → Webhooks → your endpoint → "Listen to"):

| Event | Handled by | Purpose |
|---|---|---|
| `customer.subscription.created` | `syncSubscription` | A new subscription exists -- re-derive and persist the organization's current entitlement. |
| `customer.subscription.updated` | `syncSubscription` | Any status/period/cancellation change on an existing subscription. |
| `customer.subscription.deleted` | `syncSubscription` | A subscription was canceled/deleted. |
| `invoice.paid` | `activateOnPayment` | The one durable, signed "this organization's billing has genuinely, actually succeeded at least once" signal -- sets `activated_at` (Finding 5). Deliberately NOT inferred from any subscription status snapshot. |

No other event type needs a subscription -- everything else is either irrelevant to billing state or already fully covered by the events above (`default` in the route's own `switch` statement acknowledges and ignores anything unrecognized, rather than erroring).

### Stripe Tax / Hungarian VAT invoicing — not yet decided, an owner/accountant action

`assertStripeConfigurationValid()` (above) checks that each configured Stripe Price has `tax_behavior: "inclusive"` — this proves only that the Price object's own field agrees with the billing page's own copy ("Az árak az ÁFát tartalmazzák," i.e. "prices include VAT"), a structural sanity check on the Price *configuration*. **It is not, and must never be represented as, proof that a compliant Hungarian VAT invoice is actually calculated, collected, and issued for a real purchase.** Whether the displayed price includes VAT and whether this app's Stripe setup actually produces a correct invoice for a Hungarian customer (or a customer elsewhere in the EU) are two different questions, and only the first is currently checked by anything in this codebase.

This is a genuine open item, not a technical detail this document can resolve on its own — it needs an owner/accountant decision among (at least) three options, each with real setup and ongoing-compliance implications this document does not attempt to weigh for the owner:
1. **Stripe Tax** — Stripe's own automated tax calculation/collection/reporting product. Requires enabling and configuring it per Stripe's own onboarding (registering the correct tax jurisdictions, confirming Hungarian VAT registration status), and Stripe fees for the feature.
...
`app/api/admin/reconcile-billing-sweep/route.ts` (`POST`, public in `proxy.ts`'s `PUBLIC_PATHS`) is the scheduled backstop that guarantees eventual convergence for an organization whose billing state a webhook never successfully reconciled (see `DECISIONS.md`'s "Stripe webhook reconciliation" entry) — it scans every `organization_billing` row marked `needs_reconciliation` and re-derives each one's entitlement from Stripe via the same shared service every other billing entry point uses. It refuses to run (503) if `RECONCILE_SWEEP_SECRET` isn't set, and refuses any request whose `Authorization: Bearer <secret>` header doesn't match (401) — this bearer token, not `proxy.ts`'s auth gate, is the route's actual security boundary, the identical pattern already used for the Stripe webhook route's own signature check.
...
**A fifth secret, unrelated to `check-e2e-secrets` and the four above, belongs to a different workflow entirely (fourth independent review, Finding 5 — not yet configured; an owner action):**

| Secret | Value | Used by |
|---|---|---|
| `RECONCILE_SWEEP_SECRET` | the **same** value as the `RECONCILE_SWEEP_SECRET` Vercel Production env var (§ 3) | `.github/workflows/reconcile-billing-sweep.yml` (repo root, not `ci.yml`) |

Repository Settings → Secrets and variables → Actions → **Variables** tab (not Secrets — neither of these is sensitive):
...
**Both commands are safely resumable.** Re-running `prepare` after it already fully or partially succeeded is a no-op for whatever already applied and simply finishes the rest — it does not demand that an expand migration still be pending just because a fresh run would expect that. Re-running `finalize` after it already fully succeeded is likewise a no-op (nothing pending, nothing to apply), not an error. What remains a hard failure in both is anything *unexpected*: an `--enforce` migration that's somehow already applied during `prepare` (anomalous, since `prepare` never includes `--enforce` migrations in what it pushes at all — see below), or anything pending during `finalize` that isn't in `--enforce`.

**`prepare` never touches `supabase/migrations/` itself, so a hard interruption (a killed process, a dropped connection, a crashed machine) leaves the real repository completely untouched.** Found during a second independent review: the original implementation temporarily moved the `--enforce` migration files OUT of the real directory, ran `supabase db push`, and moved them back in a `finally` block — a crash between those two steps left this repository's own working tree missing real, tracked migration files, which is a materially worse failure than "just re-run prepare," and undocumented to recover from (a manual `git checkout -- supabase/migrations/`). `prepare` now assembles a complete, throwaway temporary Supabase project directory instead (its own `config.toml`, its own `migrations/` containing every file except the staged-out `--enforce` set) and points `supabase db push --workdir` at that copy. The worst a crash can leave behind now is an orphaned OS temp directory — harmless, and nothing this repository's own files ever reflect.

### Current pending migration set (verify before every real rollout — this goes stale the moment a further migration lands)

**This example lists every migration actually still pending against production as of this PR** (a FOURTH independent-review round on the PR #3/PR #4 integration) — migrations 1 through 17 (everything through round 3) were already applied in the "Merged to production (2026-09-05)" rollout `STATUS.md` records; earlier versions of this example listed only migrations 18–23, then 18–25, then 18–28, then 18–29, each time omitting migrations added by a later round — corrected here to include all fourteen of 18 through 31 (migrations 30/31, the fourth independent review's reconciliation-lease and `billing_anomalies` additions, are the newest two), all of which must be applied before the new code that reads their columns/writes their tables (or calls their RPCs) is promoted. Re-derive this list from `supabase migration list --db-url "$PRODUCTION_DB_URL"` before running the real commands, rather than trusting this example to stay current as further rounds land.

**Migration 18 (`20260905193325`) now defines `reserve_notification_email_change`, not `request_notification_email_change`** — a third independent review found the original 2-argument function shared its name with production's already-live 3-argument `request_notification_email_change(bigint, text, int default 1440)` (migration 16). Since a call naming just `(p_organization_id, p_email)` — the only way this app has ever invoked it — matches BOTH via the 3-argument one's own default, PostgREST could not resolve the call at all (`PGRST203`) for the entire window between `prepare` (which added the 2-argument function) and `finalize` (which drops the 3-argument one) — a guaranteed outage for every caller, old and new code alike, not a bounded risk. Renaming the new function closes this completely: the two names can never collide, at any point in the rollout, so `20260906090000`/`20260906100000` below remain the correct (and now genuinely safe) way to retire the legacy function once new code no longer needs it. See `supabase/migrations/20260905193325`'s own header comment and `DECISIONS.md` for the full incident, and `e2e/rollout-compatibility-notification-email-rpc.spec.ts` for an executable proof against the real isolated project's PostgREST layer.
...
**Why `20260907150000` through `20260907240000` (the billing tables, and their grandfathering/event-ordering/rename/`activated_at`/checkout-attempt/reconciliation-lease columns and RPCs, plus `private.billing_anomalies` and its RPC) must all be `--expand`, for the identical reason as `20260906130000` above** — found during four successive independent reviews that each caught manifest entries the previous version was still missing: `app/dashboard/layout.tsx`'s paywall queries `organization_billing`, including `grandfathered_at` (`20260907150000`/`160000`) and `activated_at` (`20260907180000`), on **every** non-billing dashboard route once this PR's code is live; the webhook and every other reconciliation entry point (`features/billing/reconcile.ts`) call `claim_reconciliation_lease`/`write_reconciliation_result`/`write_activation`/`release_reconciliation_lease`/`clear_reconciliation_dirty` (`20260907210000`/`230000`/`240000` — these replace `20260907200000`'s own `claim_billing_sync`, dropped by `20260907210000`) on essentially every reconciliation attempt, and `record_billing_anomaly` (`20260907220000`) whenever an anomaly is detected; `createCheckoutSessionAction` calls `claim_checkout_attempt`/`record_checkout_session`/`release_checkout_attempt`/`renew_checkout_attempt` (`20260907200000`/`210000`) on every checkout attempt. If any of these hasn't applied yet, the corresponding request fails outright with a missing-relation, missing-column, or missing-function error, not a graceful fallback. Most of these eleven migrations are purely additive (`create table`, `alter table ... add column`, new functions, a backfill `insert`) — safe by construction, since old code simply never references a column/table/function it doesn't know about. Migration `20260907210000`'s own `drop column billing_sync_seq`/`drop function claim_billing_sync` are likewise additive-in-effect here, for the reason `DATABASE_SCHEMA.md` § "Rollout ordering" explains in full: nothing outside this still-entirely-pending migration set has ever read or called either, since `billing_sync_seq`/`claim_billing_sync` were themselves only introduced by `20260907200000`, still pending — this is not the migrations-19/20 case of old *deployed* code needing time to stop depending on something before it's removed. `20260907230000`/`240000` are `create or replace`/new-function-only additions found by this same round's own adversarial audit of the lease design (see `DECISIONS.md`) — additive for the identical reason.

`20260907170000`'s column **rename** (`last_synced_event_created_at` → `last_synced_at`) is a different case, and a second independent review found describing it as "purely additive" alongside the others was itself wrong: a rename is NOT generically safe the way an added column is — any code still referencing the OLD name breaks immediately once it applies, exactly the compatibility hazard `--enforce`'s later timing exists to protect against. This specific rename only avoids that hazard for a narrower reason: the column being renamed was itself never live in production — `organization_billing` (and every column on it, including this one) is created earlier in this SAME still-pending migration set (`20260907150000`), so no currently-deployed code path has ever read or written either name. It is `--expand`-safe here because nothing depends on it yet, not because renames are inherently additive; a rename touching a column any deployed code already reads would need `--enforce`'s expand/deploy/enforce treatment like any other breaking schema change.
