...
Hosted on Vercel, auto-deployed from pushes to `master` via Vercel's GitHub integration — **not** run by hand from a local `vercel` CLI invocation, which would deploy whatever's on disk rather than what CI has actually verified. Production: https://velemenytap.vercel.app — the original, misspelled name, which is still the live host (measured 2026-09-12: it serves 200, and `velemenytap.vercel.app` returns 404). The repository subdirectory was renamed `veleminytap/` -> `velemenytap/` on `feature/billing-subscriptions`, so Vercel's **Root Directory** setting has to change in the same window as that merge or the build fails immediately — see `DEPLOYMENT.md` § 2 and `LAUNCH_CHECKLIST.md` § 4.
