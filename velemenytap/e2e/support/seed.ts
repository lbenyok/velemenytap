...
>(fn: () => PromiseLike<T>): Promise<T> {
  ...
  for (
    let attempt = 0;
    attempt < 2 && result.error?.code === "PGRST303";
    attempt++
  ) {
 ...
 */
export async function seedFeedbackFixture(
  orgId: number,
  namePrefix: string
): Promise<SeededFeedbackFixture> {
...
export async function seedReviewGatingOrg(): Promise<SeededOrg> {
  ...
  const { data: rateLimitCard, error: rateLimitCardError } =
...
export async function cleanupOrgWithMember(
  userId: string,
  orgId: number
): Promise<void> {
  const admin = adminClient()
...
export async function signInOutcome(
  ...
  const key = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY
  if (!url || !key)
    throw new Error("NEXT_PUBLIC_SUPABASE_URL/PUBLISHABLE_KEY are required.")
  const client = createClient<Database>(url, key, {
    auth: { autoRefreshToken: false, persistSession: false },
  })
