...
## Remote NFC management — 2026-09-19

Businesses use **NFC kártyák → Deaktiválás / Aktiválás** to control their cards remotely. The physical tag stores the permanent URL generated by the dashboard (NEXT_PUBLIC_SITE_URL/r/<public_id>); status changes never rewrite the tag or change that UUID. Use the configured dashboard domain, not the Shopify .hu storefront. Existing feedback remains available after deactivation. An inactive card or location shows a neutral message without the form; a previously opened form cannot save after deactivation commits. Reactivation restores the same URL. Moving cards, lost/revoked states, bulk operations and raw scan analytics are not part of this release.
