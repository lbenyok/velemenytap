...
**Post-handoff addendum (commit `1e34282`, after `67a8a90`):** CI's own first real run against the now-mandatory `SUPABASE_DB_URL` (R4-04) surfaced a genuine environment issue this review pass hadn't hit locally — GitHub Actions runners have no IPv6 egress, and this project's direct Postgres host is IPv6-only in this region, so the direct connection string failed with `ENETUNREACH`. The fix is to use Supabase's connection pooler (IPv4-reachable) instead, which required extending `e2e/support/db-connection.ts`'s project-ref validation to also recognize the pooler URL form (previously it only matched the direct host's hostname pattern and would have wrongly rejected a pooler URL as an unapproved project). Not a review finding — an infrastructure fix needed to make R4-04's own CI enforcement actually pass. Verified: CI re-ran green after the fix, 90/90 e2e tests passed with 0 skipped (confirming the two direct-connection specs, `rpc-privilege-matrix` and `location-deactivation-race`, genuinely ran). See `DEPLOYMENT.md` § 3 for the documented pooler requirement.
