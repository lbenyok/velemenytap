...
**Confirmed directly in CI, not just reasoned about:** after pushing this round's commits to `fix/round4-review-findings`, GitHub Actions' own `Playwright e2e (review-gating regression)` job ran against commit `8885b14` under CI's actual `workers: 1` (serial), `retries: 1` profile and **completed with a `success` conclusion** (`.github/workflows/ci.yml`'s run for that commit, job id `101367194738`) — the exact execution profile this section previously flagged as untested came back green on the first real run after these changes, not merely argued to be safe.
...
- **The local full-suite Auth rate-limit exhaustion (§ 4) has since been checked against a real CI run and did not reproduce there** — CI's `Playwright e2e` job passed on commit `8885b14` under its actual serial/`retries: 1` profile. This is one observation, not a guarantee against a busier day on Supabase's shared rate limiter; if the same `AuthApiError` pattern ever does show up in CI, the fix is almost certainly raising the isolated test project's Auth rate limit (Supabase dashboard → Authentication → Rate Limits) for that project specifically — never production's — an external configuration change this repository cannot make.
...
1. **The Auth rate-limit finding in § 4/§ 5** — CI passed on the one real run observed so far, but this PR's own new tests (R5-12's concurrency tests) are part of what pushed local full-suite volume over the limit in the first place; judge whether one green CI run is enough confidence, or whether the isolated project's rate limit should still be raised proactively rather than left to a future flake.
