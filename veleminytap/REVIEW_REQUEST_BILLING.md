# Review Request — Subscription Billing (ARCHIVED)

**This document is historical, not current.** It was the live handoff for the subscription-billing feature's own review, built independently of and in parallel with `fix/round4-review-findings` (PR #3) on a separate branch, `feature/billing-subscriptions` (PR #4). That branch has since been merged into PR #3's final state as part of an integration round (see `STATUS.md`'s entry for that round, and `REVIEW_REQUEST.md` for the current handoff) — three additional safety findings (A/B/C, described in that round's `STATUS.md` entry) were fixed on `feature/billing-subscriptions` before the merge. **For the current handoff, read `REVIEW_REQUEST.md`.** Kept here, renamed, only because its billing-feature-specific detail (exact commands, exact test counts, the manual Stripe Checkout verification) remains useful historical reference for what this feature looked like at that point — nothing below should be treated as describing the repository's current state.
...
**What an independent review of this state actually found (3 findings, all confirmed and fixed before merging into PR #3):**
- **A.** Every organization that already existed before this migration ran had no `organization_billing` row at all (the provisioning trigger only fires on new-organization `INSERT`) — permanently locking it out of the dashboard, and making a checkout attempt from that state create a real, never-recoverable Stripe customer. Fixed with a `grandfathered_at` backfill — see `DECISIONS.md`.
- **B.** The webhook's idempotency record and its billing-state update were two independent, unchecked Supabase requests — a crash or transient failure between them could permanently lose a paid subscription event while looking like success. Fixed by reordering (apply then record) and adding out-of-order-delivery protection — see `DECISIONS.md`.
- **C.** Checkout had no server-side guard against a second subscription for an already-subscribed organization, and no Stripe idempotency key. Fixed — see `features/billing/actions.ts`.

Full detail, exact commits, and verification results for these three fixes are in `STATUS.md`'s entry for the PR #3/PR #4 integration round.
