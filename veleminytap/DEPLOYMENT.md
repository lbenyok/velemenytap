...
- **`features/billing/stripe-config.ts`'s `assertStripeConfigurationValid()`** — called before every Checkout attempt (`createCheckoutSessionAction`). Verifies `STRIPE_SECRET_KEY`'s own `sk_live_…`/`sk_test_…` prefix agrees with `APP_ENV`, and that both `STRIPE_PRICE_ID_MONTHLY`/`STRIPE_PRICE_ID_YEARLY` resolve (via a live `prices.retrieve` call) to Prices whose own `livemode` field also agrees, are `active`, charge exactly what the billing page promises (5,990 Ft / 59,900 Ft, `huf`, `month`/`year` recurrence), have `tax_behavior: "inclusive"` (matching the page's "az árak az ÁFát tartalmazzák" copy), and both resolve to the SAME Stripe Product (this app sells one product, two billing cadences — see `features/billing/plans.ts`). Memoized per server process; a failure is never cached, so a genuine misconfiguration keeps failing loudly on the next attempt rather than being silently retried into success once fixed.
- **`app/api/webhooks/stripe/route.ts`'s `checkEventEnvironment()`** — called on every webhook delivery. Verifies the event's own `livemode` field agrees with `APP_ENV` before processing anything; a mismatch is rejected with HTTP 400 and logged loudly, never silently ignored or silently applied. `APP_ENV` unset (local development) skips both checks — no real Stripe traffic reaches a local dev server outside `stripe listen --forward-to`.
...
**This example lists every migration actually still pending against production as of this PR** (a THIRD independent-review round on the PR #3/PR #4 integration) — migrations 1 through 17 (everything through round 3) were already applied in the "Merged to production (2026-09-05)" rollout `STATUS.md` records; earlier versions of this example listed only migrations 18–23, then 18–25, then 18–28, each time omitting migrations added by a later round — corrected here to include all seven of 23 through 29, all of which must be applied before the new code that reads their columns/writes their tables (or calls their RPCs) is promoted. Re-derive this list from `supabase migration list --db-url "$PRODUCTION_DB_URL"` before running the real commands, rather than trusting this example to stay current as further rounds land.
...
**Why `20260907150000` through `20260907200000` (the billing tables, and their grandfathering/event-ordering/rename/`activated_at`/checkout-attempt/reconciliation-fencing columns and RPCs) must all be `--expand`, for the identical reason as `20260906130000` above** — found during three successive independent reviews that each caught manifest entries the previous version was still missing: `app/dashboard/layout.tsx`'s paywall queries `organization_billing`, including `grandfathered_at` (`20260907150000`/`160000`) and `activated_at` (`20260907180000`), on **every** non-billing dashboard route once this PR's code is live; the webhook writes `last_synced_at` (`20260907170000`) on every event it processes and calls `claim_billing_sync` (`20260907200000`) on every subscription-event sync; `createCheckoutSessionAction` calls `claim_checkout_attempt`/`record_checkout_session`/`release_checkout_attempt` (`20260907200000`) on every checkout attempt. If any of these hasn't applied yet, the corresponding request fails outright with a missing-relation, missing-column, or missing-function error, not a graceful fallback. Most of these six migrations are purely additive (`create table`, `alter table ... add column`, new functions, a backfill `insert`) — safe by construction, since old code simply never references a column/table/function it doesn't know about.
