...
**Step 1 — before merging, run `prepare`:**
...
This applies only the `--expand` migrations and then verifies exactly the `--enforce` set remains pending. It touches no application code and needs no commit SHA, so it's safe to run before the merge even exists — by the time Vercel starts deploying the new code, the column/function/trigger it depends on is already there.

**Step 2 — merge and deploy, then read back the real SHA.** Merge the PR through GitHub as normal (squash or merge commit, whichever this repo's convention is) and let Vercel's auto-deploy run. Once it's merged, get the **actual** resulting commit on `master` — e.g. `git rev-parse origin/master` after fetching, or the merge commit SHA GitHub reports on the PR — never the pre-merge branch HEAD, which a squash/merge will not match.

**Step 3 — before running `finalize`, `APP_ENV` must already be set in Vercel for this environment.** This is a required human prerequisite, not an optional cleanup item (§ 3, round-6 R6-06): `/api/health` returns a fail-closed 503 without it, `finalize`'s own health poll can never succeed against a 503, and the script has no way to set this for you — only to wait on it and eventually time out less helpfully than simply doing it first. Confirm in Vercel Project Settings → Environment Variables that `APP_ENV` is scoped `production`/`preview` per § 3's table before proceeding.

**Step 4 — run `finalize` with the real SHA:**
...
node scripts/rollout.mjs finalize \
  ...
  --db-url "$PRODUCTION_DB_URL" \
  --enforce 20260906090000_fix_confirm_toctou_and_revoke_leaked_token_grant.sql,20260906100000_drop_ambiguous_request_notification_email_change_overload.sql \
  --expected-sha "<the actual post-merge SHA from step 2>"
```

`finalize` first verifies that only the `--enforce` migrations are still pending — **it refuses outright if anything else is pending** (an expand migration that never finished, most likely meaning `prepare` didn't run, ran against a different database, or was itself interrupted). Only after that check passes does it poll `/api/health` until it reports `--expected-sha` in the target's expected environment (failing closed, never proceeding, if that doesn't happen within the timeout), wait a drain window, re-check health once more, and **only then** apply the `--enforce` migrations, finishing with a check that nothing is left pending.
...
**This example lists only migrations that are actually still pending against production as of this PR** (round 8) — migrations 1 through 17 (everything through round 3) were already applied in the "Merged to production (2026-09-05)" rollout `STATUS.md` records; an earlier version of this example listed several of them (`20260904194200`/`300`/`400`) as if they were still pending, which they are not (round-7 finding R7-08). Re-derive this list from `supabase migration list --db-url "$PRODUCTION_DB_URL"` before running the real commands, rather than trusting this example to stay current as further rounds land.
