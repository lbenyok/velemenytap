# Review Request — Round 8 (ARCHIVED)

**This document is historical, not current.** It was the live handoff for round 8's own review (head `a7aa906`) but was left in place at `REVIEW_REQUEST.md` long after subsequent work (the onboarding tour, and every review-response round against it since) moved the actual current state well past everything described below — a real problem an independent review caught: reviewers are told to read `REVIEW_REQUEST.md` first, and this file's own text claimed to reflect "the current, post-round-7-fix state" long after that stopped being true. **For the current handoff, read `REVIEW_REQUEST.md`** (the current file at that path, not this one) and `STATUS.md`'s round-by-round entries, listed newest first. Kept here, renamed, only because its round-8-specific detail (exact commands, exact test counts, exact CI run at the time) remains useful historical reference for that round specifically — nothing below should be treated as describing the repository's current state, however many rounds have followed by the time this is read.
